...
export interface GlowInputProps {
  /** Floating label text that springs above the input on focus or when a value is present */
  label?: string;
  ...
  placeholder?: string;
  /** Error message — locks glow to soft coral pastel and renders message below the input */
  error?: string;
  /** Disables the input and suppresses all glow and spring motion */
  disabled?: boolean;
...
/**
 * Text input with a floating label that springs on focus and a cursor-mapped
 * pastel border glow. Label and glow share the same colour source — they feel
 * like one light system.
 *
 * @example
 * <GlowInput label="Email address" type="email" value={email} onChange={setEmail} />
 * <GlowInput label="Password" type="password" error="Incorrect password" value={pw} onChange={setPw} />
 */
export const GlowInput = ({
  ...
  useEffect(() => {
    ...
    let smoothX = 50;
    let smoothY = 50;
    let proximity = 0; // 0–1, how close cursor is to the input

    const onMove = (e: PointerEvent) => {
      rawX = (e.clientX / window.innerWidth) * 100;
      rawY = (e.clientY / window.innerHeight) * 100;

      // Calculate proximity to this input for hover glow
      if (wrapRef.current) {
    ...
    const tick = () => {
      ...
      // Input — border colour from proximity, outer glow only on focus
      if (inputRef.current) {
        if (glowClamped < 0.005) {
          inputRef.current.style.boxShadow = "0 0 0 1px rgba(255,255,255,0.1)";
        } else {
  ...
  return (
    ...
    >
      <input
        ...
        className="w-full bg-transparent border border-white/[0.15] rounded-xl px-4 pt-7 pb-3 text-white text-sm outline-none disabled:cursor-not-allowed"
        style={{
          transition: "none",
